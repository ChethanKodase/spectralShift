...
Gemma3 port of qwen/QwenUntargted_ChosenSingulaeVectors.py. Same attack, same
importance-sampling mechanism, applied to Gemma3's architecture instead of
Qwen2.5-VL's. Model loading and image preprocessing are taken from
gemma_attack/gemma3Inference.py; the module layout (vision tower, language
decoder, multi-modal projector) is taken from gemma_attack/model_parameters.txt.

For every targeted linear operator (attention q/k/v -- per attention head --
plus attention output proj, every MLP projection, and the vision-to-language
projector), this script:
...
(all derived from gemma_attack/model_parameters.txt):

  - Vision tower (SigLIP, "vision_tower.vision_model.encoder.layers.N"):
    query/key/value are three SEPARATE nn.Linear modules
    (self_attn.q_proj/k_proj/v_proj, each 1152x1152), unlike Qwen2.5-VL's
    fused attn.qkv -- so no row-slicing is needed here, they're handled
    exactly like language q/k/v (per-head SVD on each Linear directly).
    Attention output proj is self_attn.out_proj (not attn.proj). The
    vision MLP is a plain 2-layer GELU FFN, mlp.fc1 (1152->4304) / mlp.fc2
    (4304->1152) -- NOT a 3-way SwiGLU gate/up/down like Qwen's vision MLP.

  - Language decoder ("language_model.model.layers.N"): structurally the
    same SwiGLU MLP (gate_proj/up_proj/down_proj) and GQA attention
    (q_proj/k_proj/v_proj/o_proj) as Qwen, BUT Gemma3 decouples head_dim
    from hidden_size/num_heads (q_proj is (2048,2560) with 8 heads ->
    head_dim=256, while hidden_size=2560 -- 2560/8=320 != 256). This port
    therefore derives each attention projection's per-head dimension
    directly from that projection's own weight shape
    (out_features // num_heads_for_that_projection) at collection time,
    rather than assuming head_dim = hidden_size / num_heads like the Qwen
    version could safely do (Qwen's numbers happen to divide evenly).

  - Vision-to-language bridge: Gemma3 has no separate merger sub-module
    with its own nn.Linear layers (unlike Qwen's visual.merger.mlp.0/2).
    Instead there is a single raw parameter,
    multi_modal_projector.mm_input_projection_weight, shape
    (vision_hidden=1152, text_hidden=2560), used as `vision_feats @ W`
    (i.e. stored (in_features, out_features), the transpose of the usual
    nn.Linear (out,in) convention). Since a bare nn.Parameter has no
    forward_pre_hook, this port hooks the whole multi_modal_projector
    MODULE (its forward's single input is exactly the pre-projection
    vision activations we want to align) and computes the SVD of
    mm_input_projection_weight.T so it matches the (out_features,
...
for ATTACK_SAMPLE in $(seq 1 50); do
    python gemma_attack/GemmaUntargted_ChosenSingulaeVectorsMitBsa.py --attck_type ImpSampBsa --desired_norm_l_inf 0.0035 --learningRate 0.001 --num_steps 1000 --attackSample $ATTACK_SAMPLE --AttackStartLayer 0 --numLayerstAtAtime 1 --standardDivCutOff 3 --balancingAlpha 0.5
done
...
export PYTHONNOUSERSITE=1
for ATTACK_SAMPLE in $(seq 1 50); do
    python gemma_attack/GemmaUntargted_ChosenSingulaeVectorsMitBsa.py --attck_type ImpSampBsa --desired_norm_l_inf 0.0035 --learningRate 0.001 --num_steps 1000 --attackSample $ATTACK_SAMPLE --AttackStartLayer 0 --numLayerstAtAtime 1 --standardDivCutOff 4 --balancingAlpha 0.5
done
...
export PYTHONNOUSERSITE=1
for ATTACK_SAMPLE in $(seq 1 50); do
    python gemma_attack/GemmaUntargted_ChosenSingulaeVectorsMitBsa.py --attck_type ImpSampBsa --desired_norm_l_inf 0.0035 --learningRate 0.001 --num_steps 1000 --attackSample $ATTACK_SAMPLE --AttackStartLayer 0 --numLayerstAtAtime 1 --standardDivCutOff 5 --balancingAlpha 0.5
done



# Restrict to a subset of layers / fewer random noise varieties if desired:
python gemma_attack/GemmaUntargted_ChosenSingulaeVectors.py --attck_type saa_loop --desired_norm_l_inf 0.0025 --learningRate 0.001 --num_steps 1000 --attackSample 1 --standardDivCutOff 3 --numRandomVarieties 10 --chosenLanLayers 2 --chosenVisLayers 0 1 2 4 5 6 7 8 9 14 24
...
def pil_to_tensor01(pil_img: Image.Image) -> torch.Tensor:
    ...
    arr = np.array(pil_img.convert("RGB"), dtype=np.float16) / 255.0
...
# ----------------------------
# Gemma3 differentiable preprocessing
# (ported verbatim from gemma_attack/gemma3Inference.py)
# ----------------------------
...
# ----------------------------
# Generation helper
# (ported verbatim from gemma_attack/gemma3Inference.py -- no image_grid_thw,
#  Gemma3 uses a fixed-size image processor unlike Qwen2.5-VL's dynamic grid)
# ----------------------------
...
MERGER_NAME_PATTERN = re.compile(r"(^|\.)multi_modal_projector$")
...
def collect_target_specs(
    ...
    """
    Gemma3 analogue of the Qwen version's collect_target_specs. Attention
    ...
    per-head SVDs -- num_heads comes from config, but d_head and d_in are
    derived directly from each projection's own weight shape (Gemma3
    decouples head_dim from hidden_size/num_heads, so a fixed formula based
    on hidden_size would be wrong here, unlike for Qwen).
    """
    ...
    for name, module in model.named_modules():
        ...
        if lan_idx is not None and "vision_tower" not in name:
            ...
            base = dict(hook_name=name, module=module, kind="language", layer_idx=lan_idx,
                        weight_slice=None, weight_attr="weight", transpose_weight=False)
        ...
        # ---------------- vision transformer blocks (SigLIP) ----------------
        ...
        # Handled separately below since multi_modal_projector.mm_input_projection_weight
        # is a bare nn.Parameter, not an nn.Linear submodule with a ".weight" attribute --
        # it will never be matched by the named_modules() scan above.
    ...
    if include_merger:
        ...
        if hasattr(projector, "mm_input_projection_weight"):
            specs.append(dict(
                ...
                hook_name="multi_modal_projector",
                module=projector,
                kind="merger",
                ...
                is_head=False,
                weight_slice=None,
                weight_attr="mm_input_projection_weight",
                # mm_input_projection_weight is stored (in_features, out_features)
                # (used as `x @ W`), the transpose of the (out,in) convention every
                # other target's ".weight" follows -- transpose it so
                # compute_full_vh_for_spec sees a normal (out,in) matrix.
...
def compute_full_vh_for_spec(spec):
    with torch.no_grad():
        W = getattr(spec["module"], spec.get("weight_attr", "weight")).detach().to(torch.float16)
        if spec.get("transpose_weight", False):
            W = W.T
...
def run_importance_sampling(
    ...
    def run_one_pass(delta):
        ...
        pv = gemma_preprocess_differentiable(x_p01, processor)
        inputs = {k: (v.clone() if torch.is_tensor(v) else v) for k, v in template_inputs.items()}
        ...
        for hn in unique_hook_names:
            if hn in layer_inputs:
                ...
                snapshot[hn] = H.detach().to(torch.float16).cpu()
        return snapshot
...
def adam_attack_original_space(
    ...
    for step in range(num_steps):
        ...
        lossBSA = get_bsa_loss(outputs, outputsN) #+ get_bsa_loss(vision_out, vision_outN)
        ...
        attack_loss = loss * (1-balancingAlpha) + lossBSA * balancingAlpha
        ...
        if lv < best_loss:
            best_loss = lv
            best_delta = delta.detach().clone()
            np.save(save_conv_path, np.array(losses_list, dtype=np.float16))
...
def main():
    parser = argparse.ArgumentParser(
        description="Gemma3 ORIGINAL-image-space adversarial attack, importance-sampled singular-vector subspaces, ALL layer types, ALL blocks"
    )
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=None,
        help="Space-separated language layer indices to attack. Omit for ALL language layers (0-33).",
    )
    parser.add_argument(
        "--chosenVisLayers",
        type=int,
        nargs="+",
        default=None,
        help="Space-separated vision layer indices to attack. Omit for ALL vision blocks (0-26).",
    )
    parser.add_argument(
        "--includeMerger",
        dest="includeMerger",
        action="store_true",
        default=True,
        help="Include the vision-to-language multi_modal_projector target (default: on).",
    )
    parser.add_argument(
        "--no-includeMerger",
        dest="includeMerger",
        action="store_false",
        help="Exclude the vision-to-language multi_modal_projector target.",
    )
    ...
    # Preserve gemma_attack paths (see gemma_attack/gemma3Inference.py).
    ...
    print("\n[INFO] Gemma3 ALL-layer, ALL-block, CHOSEN-singular-vector variant.")
    ...
    conv_path = (
        ...
        f"balancingAlpha_{balancingAlpha}"
