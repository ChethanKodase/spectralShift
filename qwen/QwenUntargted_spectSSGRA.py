...
for ATTACK_SAMPLE in $(seq 1 50); do
    python qwen/QwenUntargted_spectSSGRA.py --attck_type saa_loop --desired_norm_l_inf 0.0025 --learningRate 0.001 --num_steps 1000 --attackSample $ATTACK_SAMPLE --AttackStartLayer 0 --numLayerstAtAtime 1 --towardsNull 0.5 --whichMLP gate_proj --whichMLPVis gate_proj --chosenLanLayers 2 --chosenVisLayers 0 1 2 4 5 6 7 8 9 14 24
done
for ATTACK_SAMPLE in $(seq 1 50); do
    python qwen/QwenUntargted_spectSSGRA.py --attck_type saa_loop --desired_norm_l_inf 0.0035 --learningRate 0.001 --num_steps 1000 --attackSample $ATTACK_SAMPLE --AttackStartLayer 0 --numLayerstAtAtime 1 --towardsNull 0.5 --whichMLP gate_proj --whichMLPVis gate_proj --chosenLanLayers 2 --chosenVisLayers 0 1 2 4 5 6 7 8 9 14 24
done
for ATTACK_SAMPLE in $(seq 1 50); do
    python qwen/QwenUntargted_spectSSGRA.py --attck_type saa_loop --desired_norm_l_inf 0.0045 --learningRate 0.001 --num_steps 1000 --attackSample $ATTACK_SAMPLE --AttackStartLayer 0 --numLayerstAtAtime 1 --towardsNull 0.5 --whichMLP gate_proj --whichMLPVis gate_proj --chosenLanLayers 2 --chosenVisLayers 0 1 2 4 5 6 7 8 9 14 24
done
...
# ============================================================
# Gemma-style selected language + vision module attack for Qwen
# ============================================================
layer_inputs = {}
...
def is_language_target(name: str, chosen_lan_layers_set=None, whichMLP="gate_proj") -> bool:
    ...
    suffix = valid_suffixes.get(whichMLP, ".mlp.gate_proj")
...
'''def is_vision_target(name: str, chosen_vis_layers_set=None) -> bool:
    ...
    # Closest Qwen analogue to Gemma vision MLP fc2 is visual.blocks.N.mlp.fc2.
    if name.endswith(".mlp.fc2"):
        return True

    # Some Qwen-VL variants use gate/up/down names in vision MLPs.
...
def collect_target_modules(model, chosen_lan_layers=None, chosen_vis_layers=None, whichMLP="gate_proj", whichMLPVis="down_proj"):
    ...
    for name, module in model.named_modules():
        ...
        if is_language_target(name, chosen_lan_layers_set=chosen_lan_layers_set, whichMLP=whichMLP):
            targets.append({
                "name": name,
                "module": module,
                "kind": "language",
...
def compute_bottom_singular_subspace(weight: torch.Tensor, towardsNull: float):
    """
    Same bottom-k selection logic as the Gemma attack:
...
def adam_attack_original_space(
    ...
    """
    Qwen version of the Gemma-style attack:
      - optimize delta in original image space [0,1]
      - differentiably preprocess to Qwen patch pixel_values
      - hook selected language and vision modules
      - minimize alignment loss toward bottom singular subspace
    """
    ...
    if len(target_specs) == 0:
        raise RuntimeError(
            "No target modules found. Check --chosenLanLayers, --chosenVisLayers, and --whichMLP."
        )
...
# ----------------------------
def main():
    parser = argparse.ArgumentParser(description="Qwen2.5-VL ORIGINAL-image-space adversarial attack, Gemma-style selected language + vision layers")
    ...
    parser.add_argument("--num_steps", type=int, default=None, help="Number of Adam steps, Gemma-style name")
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=None,
        help="Space-separated vision layer indices to attack, e.g. --chosenVisLayers 11 12 13",
    )
    ...
    print("\n[INFO] Qwen version attacks SELECTED target layers jointly, Gemma-style.")
    ...
    # Keep Qwen folders, but use Gemma-style file naming.
    conv_path = (
