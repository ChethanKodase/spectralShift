...
Sophisticated variant of qwen/QwenUntargted_ChosenSingulaeVectorsStrictCutoff.py
with a more sophisticated TOP / MIDDLE / BOTTOM split-based importance-sampling
selection rule replacing the old flat "cutoff over the whole spectrum" rule.
(Same change applied to the Gemma3 side in
gemma_attack/Gemma3_ChosenSingulaeVectorsSplittedStrictCutoff.py.)

Identical to QwenUntargted_ChosenSingulaeVectorsStrictCutoff.py in every
respect except how singular-vector indices are chosen inside
run_importance_sampling. Everything else -- Qwen2.5-VL model loading,
differentiable image patchification, target discovery (fused vision
attn.qkv row-sliced per head, language q/k/v/o + MLP gate/up/down, vision
MLP gate/up/down, merger), full-basis SVD, Adam attack loop -- is unchanged.
...
for ATTACK_SAMPLE in $(seq 1 50); do
    python qwen/QwenUntargted_ChosenSingulaeVectorsSplittedStrictCutoff.py --attck_type svisSplitStrCut --desired_norm_l_inf 0.0025 --learningRate 0.001 --num_steps 1000 --attackSample $ATTACK_SAMPLE --AttackStartLayer 0 --numLayerstAtAtime 1 --standardDivCutOff 5
done
...
export PYTHONNOUSERSITE=1
for ATTACK_SAMPLE in $(seq 1 50); do
    python qwen/QwenUntargted_ChosenSingulaeVectorsSplittedStrictCutoff.py --attck_type svisSplitStrCut --desired_norm_l_inf 0.0025 --learningRate 0.001 --num_steps 1000 --attackSample $ATTACK_SAMPLE --AttackStartLayer 0 --numLayerstAtAtime 1 --standardDivCutOff 4
done


for ATTACK_SAMPLE in $(seq 1 50); do
    python qwen/QwenUntargted_ChosenSingulaeVectorsSplittedStrictCutoff.py --attck_type svisSplitStrCut --desired_norm_l_inf 0.0025 --learningRate 0.001 --num_steps 1000 --attackSample $ATTACK_SAMPLE --AttackStartLayer 0 --numLayerstAtAtime 1 --standardDivCutOff 10
done

for ATTACK_SAMPLE in $(seq 1 50); do
    python qwen/QwenUntargted_ChosenSingulaeVectorsSplittedStrictCutoff.py --attck_type svisSplitStrCut --desired_norm_l_inf 0.0025 --learningRate 0.001 --num_steps 1000 --attackSample $ATTACK_SAMPLE --AttackStartLayer 0 --numLayerstAtAtime 1 --standardDivCutOff 20
done
...
#
# DIFFERENCE vs QwenUntargted_ChosenSingulaeVectorsStrictCutoff.py: weak_norm
# is first split into three equal contiguous thirds -- top (largest singular
...
def main():
    parser = argparse.ArgumentParser(
        description="Qwen2.5-VL ORIGINAL-image-space adversarial attack, importance-sampled singular-vector subspaces, ALL layer types, ALL blocks, STRICT cutoff with TOP/MIDDLE/BOTTOM split-based selection"
    )
    ...
    print("[INFO] language layer, and the merger projection, weak_norm (the")
