...
for StudyLayer in $(seq 0 27); do
    python qwen/Qwen2p5FullDistancesOverlap0.py --attck_type bsa --desired_norm_l_inf 0.005 --learningRate 0.001 --num_steps 1000 --AttackStartLayer 0 --numLayerstAtAtime 1 --VisionLayerTrack 0 --LanLayerTrack $StudyLayer --kthSingVec -10 --attackMode lan
done
...
for StudyLayer in $(seq 0 31); do
    python qwen/Qwen2p5FullDistancesOverlap0.py --attck_type bsa --desired_norm_l_inf 0.005 --learningRate 0.001 --num_steps 1000 --AttackStartLayer 0 --numLayerstAtAtime 1 --VisionLayerTrack $StudyLayer --LanLayerTrack 0 --kthSingVec 10 --attackMode vis
done
...
def main():
    ...
    for attackSample in range(1,NumSamplesConsidered):
        ...
        for i in range(len(FlattenedAlignmentDistributionsOriginal)):
            ...
            diffStrong = (orig_c * 0  - adv_c).pow(2).mean(dim=token_dim).sqrt().flatten()
            diffWeak = (orig_c  * 0  - weak_c).pow(2).mean(dim=token_dim).sqrt().flatten()
    ...
    for i in range(len(averagedAggregationOverFlattenedAlignmentDistributionsOriginal)):
            ...
            save_dir = f"qwen/OverlapDistancesAvg0"
            ...
            save_dirBands = f"qwen/OverlapDistancesAvgStdBands0"
            ...
            save_dirBands = f"qwen/OverlapDistancesAvgStdBandsNormalized0"
            os.makedirs(save_dirBands, exist_ok=True)
            save_pathBands = os.path.join(
                save_dirBands,
                f"Bar_{label.replace(' ', '_')}_attackSample_{attackSample}_attackMode_{attackMode}_LanLayerTrack_{LanLayerTrack})_VisionLayerTrack_{VisionLayerTrack}.png"
